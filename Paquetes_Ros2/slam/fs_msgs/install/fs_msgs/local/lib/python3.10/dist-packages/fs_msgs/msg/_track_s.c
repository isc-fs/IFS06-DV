...
bool fs_msgs__msg__track__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("fs_msgs.msg._track.Track", full_classname_dest, 24) == 0);
  }
  fs_msgs__msg__Track * ros_message = _ros_message;
  {  // track
    ...
    if (!fs_msgs__msg__Cone__Sequence__init(&(ros_message->track), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create fs_msgs__msg__Cone__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    fs_msgs__msg__Cone * dest = ros_message->track.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!fs_msgs__msg__cone__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
PyObject * fs_msgs__msg__track__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("fs_msgs.msg._track");
  ...
  {  // track
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      PyObject * pyitem = fs_msgs__msg__cone__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
